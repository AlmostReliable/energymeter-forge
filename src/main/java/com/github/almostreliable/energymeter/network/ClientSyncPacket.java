...
public class ClientSyncPacket {
    ...
    static ClientSyncPacket decode(PacketBuffer buffer) {
        ...
        if ((packet.flags & SYNC_FLAGS.SIDE_CONFIG) != 0) {
            packet.sideConfig = Objects.requireNonNull(buffer.readNbt());
        }
        if ((packet.flags & SYNC_FLAGS.TRANSFER_RATE) != 0) packet.transferRate = buffer.readDouble();
        if ((packet.flags & SYNC_FLAGS.NUMBER_MODE) != 0) {
            packet.numberMode = NUMBER_MODE.values()[buffer.readInt()];
        }
        if ((packet.flags & SYNC_FLAGS.STATUS) != 0) {
            packet.status = STATUS.values()[buffer.readInt()];
        }
        if ((packet.flags & SYNC_FLAGS.MODE) != 0) packet.mode = MODE.values()[buffer.readInt()];
        if ((packet.flags & SYNC_FLAGS.ACCURACY) != 0) {
            packet.accuracy = ACCURACY.values()[buffer.readInt()];
        }
